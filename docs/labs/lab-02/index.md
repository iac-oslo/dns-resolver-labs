...
```
vm-workload ──DNS query──► Resolver inbound (10.10.0.4)
                                    │
                                    ▼
                    ...
                    (linked to vnet-single-norwayeast)
                                    │
                                    ▼
...
- Resolver inbound endpoint IP noted (expected: `10.10.0.4`)
...
!!! info
    Replace `10.10.0.4` with the actual inbound endpoint IP if it differs (verify in Task #3 of lab-01).
...
!!! success "What you verified"
    DNS query from vm-workload → DNS Resolver inbound endpoint (10.10.0.4) → Azure DNS → Private DNS Zone `privatelink.blob.core.windows.net` → Private Endpoint IP.
...
You should see a link to `vnet-single-norwayeast`.

!!! info "Why link the zone to the VNet?"
    When the DNS Resolver inbound endpoint receives a query for `privatelink.blob.core.windows.net`, it forwards it to Azure DNS (168.63.129.16). Azure DNS resolves Private DNS Zones that are linked to the VNet containing the resolver — which is `vnet-single-norwayeast`. This is why the Private DNS Zone must be linked to the **resolver's VNet**, not necessarily to every VNet that queries it.
