...
!!! warning "Azure Firewall and Bastion cost money even when idle"
    Azure Firewall Basic costs ~$0.33/hr and Bastion Basic costs ~$0.19/hr. A forgotten deployment left running for 12 hours can cost ~$11.74 for Set 3 alone.

## Task #1 - Delete all lab resource groups

Run the following commands to delete all three resource groups. The `--no-wait` flag returns immediately; deletion runs in the background.
