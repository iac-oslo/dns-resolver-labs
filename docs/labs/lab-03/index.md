...
In this lab you create a DNS Forwarding Ruleset using Bicep (IaC task). The ruleset tells the DNS Resolver to forward queries for `onprem.iac-labs` to the on-prem DNS server (BIND9 running on `vm-onprem-norwayeast`).

**Scenario:** A workload VM needs to resolve internal hostnames from an on-premises DNS zone (`onprem.iac-labs`). The DNS Resolver outbound endpoint forwards matching queries to the on-prem DNS server.

```
vm-workload ──dig app.onprem.iac-labs──► Resolver inbound (10.10.2.4)
                                               │
                              ...
                              vm-onprem-norwayeast BIND9 (port 53)
                                               │
                                               ▼
                                      app.onprem.iac-labs = vm-onprem IP
```
...
Resolve the `app.onprem.iac-labs` record — registered in BIND9 with the on-prem VM's IP:
...
```
;; ANSWER SECTION:
app.onprem.iac-labs.   300   IN   A   10.10.1.4
```

Resolve the synthetic `db.onprem.iac-labs` record:
...
!!! success "What you verified"
    Queries for `onprem.iac-labs` domain are forwarded by the DNS Resolver outbound endpoint to the on-prem BIND9 server. The on-prem server responds with the correct records. The workload VM never needed direct network access to a DNS server in a different zone — the resolver handles the forwarding.
...
This should still resolve to the Private Endpoint IP — the forwarding ruleset only intercepts `onprem.iac-labs.` queries.
